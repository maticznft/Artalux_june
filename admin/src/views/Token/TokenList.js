...
const ViewTokens=async(record)=>{
  ...
  if (record.tokenOwnerDb&&record.tokenOwnerDb.tokenCounts&&record.tokenOwnerDb.tokenOwner) {
    window.location.href = "/xulatra/ViewToken/" +record.tokenOwnerDb.tokenOwner +"/" +record.tokenOwnerDb.tokenCounts;
  }
  else{
    window.location.href = "/xulatra/ViewToken/" +record.tokenOwner +"/" +record.tokenCounts;
...
function editR(id) {
  ...
  if (id != '') {
    window.location.href = "/xulatra/ViewToken/" + id;
  }
...
async function deleteR(id) {
  if (id != '') {
    ...
    setTimeout(
      () => window.location.href = "/xulatra/TokenList",
      1000);
...
export default function EnhancedTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      <div><p>{record.tokenOwnerDb.clocktime!=null?moment(record.tokenOwnerDb.clocktime).format('MMMM,Do YYYY, hh:mm A'):"null"}</p></div>
    ...
    },
    {
      ...
      // //console.lo(record.date,"date")
      // <div>{record.clocktime}</div>
      <div><p>{record.tokenOwnerDb.endclocktime!=null?moment(record.tokenOwnerDb.endclocktime).format('MMMM,Do YYYY, hh:mm A'):"null"}</p></div>
    ...
    },
    {
      ...
      // //console.lo(record.date,"date")
      // <div>{record.clocktime}</div>
      <div><p>{record.tokenOwnerDb.minimumBid}</p></div>
    ...
    },
    {
      ...
      sortable: false,
      cell: record =>
      <div><p>{(record.hashValue)}</p>{console.log("record fasfasdfas",record)}</div>
  ...
  const Addtoken = async () => {
    window.location = "/xulatra/AddToken";
  }
