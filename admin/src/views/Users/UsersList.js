...
import { getuserslist, deleteToken, getuserdaily, getusermonthly, getusercustom, getcsvdata, setactivateUserAction } from '../../actions/users';
...
async function deleteR(id) {
  if (id != '') {
    ...
    setTimeout(
      () => window.location.href = "/xulatra/UsersList",
      1000);
...
export default function EnhancedTable() {
  ...
  const columns = [
    ...
    //           // {/* { record._id != "" &&  <input id = {record._id} type="checkbox" name="activeUser" checked = {record.activate} onChange={ (event) => { 
    //           //   setactivateUser(event)
    //           // }
    //           // } */}
    //           // > 
    //           // {/* } */}
  ...
  async function setactivateUser(event) {
    ...
    var activeUserData = await setactivateUserAction(payload);
    await getuserslist()
    if (activeUserData && activeUserData.data && activeUserData.data.success && activeUserData.data.success === true) {
      if (checked) {
          toast.warning('Users activated',toasterOption); 
      }else {
        toast.warning('Users deactivated',toasterOption);
      }
