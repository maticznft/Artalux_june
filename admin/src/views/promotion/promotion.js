...
export default function PROMOTIONLIST() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "CoinName",
      text: "CoinName for Mimimum Bid or Token Price",
      className: "minimumBid",
      align: "left",
      sortable: false,
      cell: record =>
     
      <div><p>{record.CoinName}</p></div>
          ...
          <Fragment>
                ...
                <Button onClick={()=>setPromotion(record)}>
                 ...
                 {/* <input type="radio"
                  value={record._id}
                   name={record._id}
               checked={(Selected12 == record._id) }
               onChange={()=>{onValueChange(record._id)}}
                 /> */}
  ...
  const getTokenlist = async () => {
    ...
    //   (test.userValue).map((item,index)=>{
    //     if(item.promotion =="show"){
    //     setSelected1(item._id);
    //     Selected123="true"+test.userValue[0]._id
    //     console.log("Sadasdsadas",Selected123,test.userValue[0]._id)}
    //   })
    //   }
  ...
  async function setPromotion(data){
    var payload ={
      _id:data._id,
    ...
    var test = await setPromotions(payload);
