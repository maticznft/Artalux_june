...
export default function EditServiceFee() {
    ...
    const getserviceFee=async()=>{
            ...
            var servicefee = await contractCall.methods.getListingFees().call()
            console.log("Service fee 12", servicefee)
            set_default_service_fee(servicefee/1e18)
            set_default_service_fee0(servicefee/1e18)
            set_buyer__fee(servicefee/1e18)
            //set_seller__fee(servicefee[1] / 1e18)
            var contractCallMul = new web3.eth.Contract(MULTIPLE, config.multipleContract)
            var servicefee1 = await contractCallMul.methods.getListingFees().call()
            console.log("Service fee", servicefee1)
            set_default_service_fee1(servicefee1/1e18)
            set_default_service_fee11(servicefee1/1e18)
            // var contractCallTrade = new web3.eth.Contract(TRADE, config.trade)
            // var servicefee2 = await contractCallTrade.methods.getListingFees().call()
    ...
    const EditServiceFeesOnly721 = async (e) => {
        ...
        if ((Number(buyer__fee)) && (Number(buyer__fee)) != 0 && (Number(buyer__fee)) < 10) {
                //alert("in2")
            setServiceDisable1('process')
            var web3 = new Web3(window.ethereum)
            var contractCall = new web3.eth.Contract(SINGLE, config.singleContract)
            console.log("set Servic Fee ", typeof parseFloat((buyer__fee)))
            var fees =web3.utils.toWei(String((Number(buyer__fee))))
            await contractCall
                .methods
                .setListingFees((web3.utils.toWei(String(buyer__fee))))
                .send({ from: Accounts })
                .then(async () => {
    ...
    const EditServiceFeesOnlytrade = async (e) => {
        if ((Number(buyer__fee)+Number(seller__fee)) && (Number(buyer__fee)+Number(seller__fee)) != 0 && (Number(buyer__fee)+Number(seller__fee)) < 10) {
            ...
            await contractCall
                .methods
                .setListingFees((web3.utils.toWei(String(buyer__fee))),web3.utils.toWei(String(seller__fee)))
                .send({ from: Accounts })
                .then(async () => {
                    if(ServiceDisable1 != 'done')
                    {
        ...
        <div>
                    ...
                    <Card>
                        <form className={classes.form}>
                            <CardHeader color="primary">
                                <h4 className={classes.cardTitleWhite}>View List Fee <span className="font-weight-bold">(All THREE must be SAME)</span>   Note - This process take <span className="font-weight-bold">Three Transaction</span> Please Confirm One by One</h4>
                            </CardHeader>
                            <CardBody>
                                <GridContainer>
                                    ...
                                    <p className="text-center">List Fee-{buyer__fee}%</p>
                                    {/* <p className="text-center">Seller Fee-{default_service_fee0}%<span className="sertext"> List fee in 721</span></p> */}
                                    </div>
                                    </GridItem>
                                        <GridItem xs={12} sm={12} md={3} >
                                        <div className="servicemod border">
                                        <p className="text-center">List Fee - {buyer__fee}%</p>
                                    {/* <p className="text-center">Seller Fee - {default_service_fee11}%<span className="sertext"> List fee in 1155</span></p> */}
                                    </div>
                                    </GridItem>
                                        {/* <GridItem xs={12} sm={12} md={3} >
                                        <div className="servicemod border">
                                        <p className="text-center">Buyer Fee - {default_service_fee2}%</p>
                                    <p className="text-center">Seller Fee - {default_service_fee21}%<span className="sertext"> List fee in Trade</span></p> */}
                                    {/* </div>
                                    </GridItem> */}
                                        ...
                                        />
                                        {buyer__fee == 0 && <span className="text-danger">Check List fee</span>}
                                        {/* <CustomInput
                                            labelText="Seller List Fee"
                                            id="servicefeecheck1"
                                            value={seller__fee}
                                            onChange={(e) => onChangeFUnc(e)}
                                            formControlProps={{
                                        ...
                                        />
                                        {seller__fee == 0 && <span className="text-danger">Check List fee</span>} */}
            ...
            >
               ...
               <div className="servicemod">
                   <p className="text-center"> List fee<span className="sertext"> List Fee - {buyer__fee} %
                   {/* % And Seller Fee - {seller__fee}% */}
                   </span></p>
                   <button
                   disabled={(ServiceDisable1 == 'process') || (ServiceDisable1 == 'done') || (ServiceDisable == 'error')}
                   onClick={EditServiceFeesOnly721}
                    >
                   {ServiceDisable1 == 'init' && ` List Fee - ${default_service_fee}% to ${buyer__fee}% 
                   ...
                   {ServiceDisable1 == 'done' && `List Fee -  ${(Number(buyer__fee))}% List fee in 721`}
                   ...
                   >
                    {ServiceDisable2 == 'init' && ` List Fee - ${default_service_fee1}% to ${buyer__fee}% 
                List fee in 1155`}
                   ...
                   {ServiceDisable2 == 'done' && `List Fee -  ${(Number(buyer__fee))}%  
         List fee in 1155`}
                   ...
                   >
                    {ServiceDisable3 == 'init' && ` Buyer Fee - ${default_service_fee2}% to ${buyer__fee}% And   Seller Fee - ${default_service_fee21}% to ${seller__fee}% List fee in Trade`}
                   ...
                   {ServiceDisable3 == 'done' && `Buyer Fee -  ${(Number(buyer__fee))}%  Seller Fee -  ${(Number(seller__fee))}% List fee in Trade`}
