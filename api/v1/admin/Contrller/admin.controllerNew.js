...
import Bpxtoken from '../../admin/Model/bpxtoken'
import fs from 'fs'
import { stdout } from 'process';
import bpxtoken from '../../admin/Model/bpxtoken';
import subscriberDb from '../../../models/subscriber';
...
export const BurnField = async (req, res) => {
  ...
  try {
   ...
   //changed
      let report = await ReportSchema.findOne({ "imageId": String(reqBody.tokenCounts), "imageOwner": String(reqBody.tokenOwner),"imageContractAddress":String(reqBody.contractAddress)});
      if (report) {
        console.log("all okkkkkkkk",typeof report.noofitems,typeof reqBody.balance)
     
         var ch1= await  ReportSchema.findOneAndUpdate({ "imageId": String(reqBody.tokenCounts), "imageOwner": String(reqBody.tokenOwner),"imageContractAddress":String(reqBody.contractAddress)} ,
        {"$set":{
        ...
        var sv=new ReportSchema({
          ...
          imageId:String(reqBody.tokenCounts),
    ...
    let checkUser1 = await tokenOwnerDb.findOne({ "tokenCounts": String(reqBody.tokenCounts), "tokenOwner": reqBody.tokenOwner,"contractAddress":reqBody.contractAddress,"type":reqBody.type });
   
   
    if (checkUser1) {
      var ch2= await   tokenOwnerDb.findOneAndUpdate({ "tokenCounts": String(reqBody.tokenCounts), "tokenOwner": reqBody.tokenOwner,"contractAddress":reqBody.contractAddress}
       ,{$set:{
...
export const addcategory = async (req, res) => {
  ...
  console.log("categoryData",req.body, categoryData)
  if (categoryData) {
...
if(req.files!=null){
  ...
  category.save(async function (err, rest) {
    if(req.files!=null){
    await fs.mkdir('public/admin/category/' + rest._id, { recursive: true }, async function (err) {
      if (err) return;
      if (File.name != ""){
       var UploadFullPath='public/admin/category/' + rest._id +'/' + rest.image
          File.mv(UploadFullPath, function (err) {
            if (err) return;
          });
        }
    });}
    return res.status(200).json({ 'success': true, 'message': "Category details added successfully" })
  ...
  var userData=await Token.aggregate([
    {
      $match:{
        // deleted:1
        "tokenCounts":String(req.params.id) //changed
      },
    },
      {
      $lookup:{
...
export const deletecategorys = async (req, res) => {
  ...
  try {
   ...
   .then((data)=>{
    ...
    return res
      .status(200)
      .json({ success: true, message: "category Updated Successfully",data:data });
    })
  } catch (err) {
    console.log("----err", err)
    return res
   ...
   //console.log("jsbdjkbejkfbeajkfbkjb",reqBody);
    ...
    if(data){
      ...
      var linksave = new Settings({
        ...
        // Website:reqBody.Website,
        //   Instagram:reqBody.Instagram,
        //   Twitter:reqBody.Twitter,Settings
        //   Telegram:reqBody.Telegram,
        //   Reddit:reqBody.Reddit,
        //   Discord:reqBody.Discord,
      })
...
export const getPrivacyVal = async (req, res) => {
  console.log("GetLink",req.body.location)
...
export const getAdminData = async (req,res) => {
    ...
    console.log("ewriuewrweiryuweyuryewyrewuyrw",req.body.currAddr)
		...
		if (adminPostData) {
      console.log('adminresp',adminPostData);
			return res
				.status(200)
...
var userData=await Token.aggregate([
  ...
  },
  {
    $project:{
      ...
      tokenCreator: 1,
      tokenRoyality: 1,
      status: '$tokenOwnerDb.status',
      hashValue: '$tokenOwnerDb.hashValue',
      type: '$tokenOwnerDb.type',
      balance: '$tokenOwnerDb.balance',
      tokenQuantity: 1,
      contractAddress: '$tokenOwnerDb.contractAddress',
      minimumBid: '$tokenOwnerDb.minimumBid',
      endclocktime:  '$tokenOwnerDb.endclocktime',
      clocktime:  '$tokenOwnerDb.clocktime',
      likecount: 1,
      PutOnSale: 1,
      PutOnSaleType: 1,
      ipfsimage: 1,
     //  unlockcontent: 1,
      ...
      OnSaleOwner: 1,
      swapPrice:1,
      promotion:'$tokenOwnerDb.promotion'
    }
  }
...
var userData=await Token.aggregate([
  ...
  },
  {
    ...
    // {  
      $and:[
        {'tokenOwnerDb.promotion':{$eq:'show'}},
        { 'tokenOwnerDb.balance':{$gt:0}},
        { 'tokenOwnerDb.deleted':{$gt:0}}
      ]}
  ...
  },
  {
    $project:{
     ...
     //  unlockcontent: 1,
      ...
      swapPrice:1,
      promotion:'$tokenOwnerDb.promotion',
      CoinName:'$tokenOwnerDb.CoinName',
      thumb:1,
...
console.log("reqdata",req.body)
  try{
    if(req.body.promotion == "hide"){
   tokenOwnerDb.findOneAndUpdate({"_id":req.body._id,promotion:"hide"},{"$set":{"promotion":"show"}},{new:true}).then((unsetSwapPrice)=>{
    
    console.log("unserSwap_1",unsetSwapPrice)
    return res
        .status(200)
        .json({ success: true, userValue: unsetSwapPrice });
    })
    }
    else if(req.body.promotion == "show"){
    tokenOwnerDb.findOneAndUpdate({"_id":req.body._id,promotion:"show"},{"$set":{"promotion":"hide"}},{new:true}).then((unsetSwapPrice)=>{

      console.log("unserSwap",unsetSwapPrice)
      return res
        .status(200)
        .json({ success: true, userValue: unsetSwapPrice });
...
console.log("ioiooioioioioiooioioioioiooooi",req.body)
  try{
    var unsetSwapPrice=await tokenOwnerDb.findOneAndUpdate({"tokenOwner":req.body.tokenOwner,"tokenCounts":String(req.body.tokenCounts)},{"$set":{"deleted":req.body.deleted}},{new:true})
    console.log("uwqqwteuyqwtetqweyqw",unsetSwapPrice)
    if(unsetSwapPrice){
      console.log("wqeuiwqueuqiweiwqiuieuqwuieuiwqueqwasasasasa1111111111111111111111111")
    return res
      .status(200)
      .json({ success: true, userValue: unsetSwapPrice });
    }
...
export const setActivateUser = async (req,res) => {
  try {
    console.log('req.body>>>>>',req.body);
    let reqBody = req.body;
    let id = reqBody.id;
    let checked = reqBody.checked;
    var activateUsers = await User.update({_id : id}, {$set : {activate : checked}}, {new : true});
    return res
      .status(200)
      .json({ success: true, list: activateUsers });
  }catch(error) {
...
export const tokenaddressAdd=async(req,res)=>{
  var nedata=new tokenaddress({
...
export const hideshows=(req,res)=>{

var ReqData=req.query;
console.log("palamurai",ReqData)
var show=ReqData.show=='false'?true:false
Category.findOneAndUpdate({_id:ReqData._id,},{$set:{show:show}},{new:true})
.then((data)=>{
...
export const bpxtokenadd=(req,res)=>{
  var ReqData=req.body;
 
    // console.log("obj key",req.body,req.files)
    var timestamp= Date.now()
var savedata={
  'tokenName':req.body.tokenName
  ,tokenAddress:req.body.tokenAddress
  ,tokenContents:req.body.tokenContents
  ,network:req.body.network
  ,ex_img:req.files&&req.files.ex_img!=null?(timestamp+req.files.ex_img.name):''
  ,ex_name:req.body.ex_name
  ,ex_last:req.body.ex_last
  ,ex_list:req.body.ex_list
  ,wl_img:req.files&&req.files.wl_img!=null?(timestamp+req.files.wl_img.name):''
  ,wl_name:req.body.wl_name,
...
//  bpxtoken.findOne({_id:req.body._id})
//   .then((data)=>{
    if(req.body._id!=''){
    

      bpxtoken.findOneAndUpdate({_id:req.body._id},{$set:savedata},{new:true}).then(async(data)=>{
        console.log("varan")
         if(req.files){
           if(req.files.ex_img!=null){
           await fs.mkdir("public/admin/bpxtoken/exchange/", { recursive: true }, function (err) {
             if (err) return;
             req.files.ex_img.mv("public/admin/bpxtoken/exchange/"+timestamp+req.files.ex_img.name, function (err) {
               if (err) return;
             });
           });}
            if(req.files.wl_img!=null){
             await fs.mkdir("public/admin/bpxtoken/wallet" , { recursive: true }, function (err) {
               if (err) return;
               req.files.wl_img.mv("public/admin/bpxtoken/wallet/"+timestamp+req.files.wl_img.name, function (err) {
                 if (err) return;
               });
             });}
           }
      ...
      var savedata1=new bpxtoken(savedata)
      // console.log("palamurai _  ",savedata)
    savedata1.save().then(async()=>{
   if(req.files){
      if(req.files.ex_img!=null){
      await fs.mkdir("public/admin/bpxtoken/exchange/", { recursive: true }, function (err) {
        if (err) return;
  ...
  export const gettokenLists =async(req,res)=>{
    bpxtoken.find({})
    .then(data=>{
      res.json(data)
    }).
  ...
  export const deletToken = async (req, res) => {
    console.log( "deleteUser ",req.params.id)
    try {
      let deleteuser = await bpxtoken.findById({ _id:req.params.id }).remove();
      console.log(deleteuser);
      return res
        .status(200)
        .json({ success: true, message: "Faq Deleted Successfully" });
  ...
  export const gettokensdatalast = async (req, res) => {
    console.log( "deleteUser ",req.params.id)
    try {
      let deleteuser = await bpxtoken.findOne({});
      console.log(deleteuser);
      return res
        .status(200)
        .json(deleteuser);
  ...
  export const edittokensdatalast = async (req, res) => {
    console.log( "deleteUser ",req.params.id)
    try {
      let deleteuser = await bpxtoken.findOne({_id:req.params.id});
      console.log(deleteuser);
      return res
        .status(200)
        .json(deleteuser);
  ...
  export const getSubscriberList = async (req,res) => {
    try {
        var data = await subscriberDb.find({});
        res.json({Success : true , data : data});
    } catch( error ) {
      console.log("Error in getSubscriberList Function",error)
      res.json({Success : false , Message : "Error on Server"});
    }
  }
  
  export const subscriberStatus = async  (req,res) => {
    try {
      let reqBody = req.body;
      let email = reqBody.email;
      let maySentChecked = reqBody.maySentChecked;
      let subscriberUpdateStatus = await subscriberDb.update({email : email} ,{$set : { maySent : maySentChecked }},{new : true});
      return res.json({Success : true , Message : 'Subscribed restricted',data : subscriberUpdateStatus});
    }catch (error) {
      return res.json({Success : false , Message : "Oops something went wrong on subscriberlist"});
    }
  }
  
  export const newsLetternew = async (req, res) => {
    try {
      let reqBody = req.body;
      let bodyOfContent = reqBody.boc;
      let subject = reqBody.subject;
      let subscribers = await subscriberDb.find({ maySent: true }, { _id: 0, email: 1 });
      ...
      console.log("Error in newsletter Function",error)
      res.status(200).json({ Success: false, Message: "Error on Server" });
    }
  }
